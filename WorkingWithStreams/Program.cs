...
static void WorkWithXml()
{
    ...
    try
    {
        // define a file to write to string
...
static void WorkWithCompression(bool useBrotli = true)
{
    ...
    FileStream file = File.Create(filePath); Stream compressor; 
    if (useBrotli) { 
        compressor = new BrotliStream(file, CompressionMode.Compress); 
    } else { compressor = new GZipStream(file, CompressionMode.Compress); } 
    using (compressor) { 
    ...
    WriteLine($"The compressed contents:"); WriteLine(File.ReadAllText(filePath)); 
        ...
        using (XmlReader reader = XmlReader.Create(decompressor)) { 
            while (reader.Read()) {
                ...
                if ((reader.NodeType == XmlNodeType.Element) && (reader.Name == "callsign")) { 
                    reader.Read(); 
                    // move to the text inside element
...
{ // define an array of Viper pilot call signs
    public static string[] Callsigns = new[] { "Husker", "Starbuck", "Apollo", "Boomer", "Bulldog", "Athena", "Helo", "Racetrack" };
}
